...
- **Update 2026-08-18, later same day**: `auth login` succeeded, and `get_weight_history` was confirmed live against the real account (#1, #2 both effectively done - real weight data came back correctly shaped/converted). Ran the actual comparison against fatsecret-mcp for August: 8/11 days missing from FatSecret, the 3 present off by 0.1-0.35 kg (Fitbit's native single-reading sync vs. Google Health's day-average). **Attempted backfill blocked by a real FatSecret platform limit, not a bug**: `weight.update` rejects any date more than 2 days in the past (error 205, undocumented on FatSecret's reference page) - all 11 writes failed. Full write-up on issue #3. Historical backfill via this API isn't possible at all; the sync use case only works going forward (run within ~2 days of each new weigh-in) or via manual entry in FatSecret's own app for the historical gap.
- Next action is on the developer: decide whether to (a) run a fitbit-mcp + fatsecret-mcp session every day or two going forward to keep future weigh-ins in sync within FatSecret's writable window, and/or (b) manually backfill the historical gap through FatSecret's app directly.
